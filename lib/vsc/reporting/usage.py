...
##
"""
Class to get the montly usage for given users
...
from vsc.config.base import GENT, GENT_PRODUCTION_COMPUTE_CLUSTERS, MAIL_SERVER
...
class UsageReport(CLI):
    ...
    def do(self, dry_run):
        ...
        mail_body = "\n".join(body)
        mail_subject = "HPC: industry usage report (hours per month) %s - %s" % (self.options.start, self.options.end)
        mail_to = self.options.recipient

        mail = VscMail(MAIL_SERVER[GENT])
        mail.sendTextMail(
            mail_to=mail_to,
            mail_from=MAIL_FROM,
            reply_to=MAIL_FROM,
